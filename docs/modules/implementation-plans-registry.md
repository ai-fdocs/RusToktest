...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `82%` | `agent` | `2026-05-24T18:34:00Z` | `в C1 добавлена bounded aggregation-модель sitemap submission (limit details + omitted-tail), regression matrix закрывает timeout/failure truncation без изменения public SeoSitemapStatusRecord` | Разбить `sitemaps` runtime на 3 дочерних подпакета (`index-generation`, `submission-adapters`, `submission-aggregation`) и вынести provider seam в отдельный runtime объект | `provider-specific adapters блокируются отсутствием tenant-secret/rotation contract` | `cargo test -p rustok-seo --lib sitemaps` |
