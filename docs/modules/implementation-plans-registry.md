...
| `rustok-tenant` | `rustok-tenant` | `crates/rustok-tenant/docs/implementation-plan.md` | `in_progress` | `65%` | `agent` | `2026-05-21T00:00:00Z` | `синхронизированы observability-метрики cache и active tenant signals` | Закрыть интеграционные тесты для resolver invariants и задокументировать provisioning/deprovisioning contract | `-` | `cargo test -p rustok-tenant --lib` |
