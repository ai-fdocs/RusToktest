...
| `rustok-comments` | `rustok-comments` | `crates/rustok-comments/docs/implementation-plan.md` | `in_progress` | `30%` | `agent` | `2026-06-07T00:00:00Z` | `Comments admin core теперь владеет status parsing и thread/detail/comment row view-model mapping; Leptos adapter потребляет core helpers поверх native-only transport facade` | Спланировать GraphQL/headless fallback parity adapter для выхода из temporary single-adapter state | GraphQL fallback для comments admin пока не реализован; FBA остается not_started до boundary evidence | `cargo check -p rustok-comments-admin --config profile.dev.debug=0`; `cargo test -p rustok-comments-admin --lib --config profile.dev.debug=0`; `npm run verify:ffa:ui:migration` |
