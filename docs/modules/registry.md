...
| `forum` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-forum/docs/implementation-plan.md` (admin+storefront slices: `core` helpers + thin `transport` facades + explicit `ui/leptos` adapters; latest admin slice moves category/topic form snapshots and submit validation into Leptos-free core; REST/native-first + GraphQL fallback contracts unchanged) |
