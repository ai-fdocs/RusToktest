...
| `forum` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-forum/docs/implementation-plan.md` (storefront slice: `core` href/status/rich-content policy + thin `transport` facade + explicit `ui/leptos` adapter; native-first + GraphQL fallback contract unchanged) |
