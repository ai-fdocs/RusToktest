...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (slice #76: admin required-field submit issue construction moved into `admin/src/core.rs` with unit-test evidence; previous slice added explicit admin/storefront `ui/leptos.rs` render adapters and storefront adapters consume core-owned fetch request) |
