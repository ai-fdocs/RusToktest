...
| `search` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-search/docs/implementation-plan.md` (slice #38: admin transport facade now accepts core-owned preview/dictionary command DTOs; Leptos adapter keeps only route-query side effects and UI reset/refresh effects; native/GraphQL fallback unchanged) |
