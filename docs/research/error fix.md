...
- Завершающий admin-copy cleanup slice убрал stale copy про оставшиеся inventory admin mutations из Leptos fallback strings и locale bundles: текущие set/adjust/reserve/release/check-availability UI operations описаны как native inventory facade, а будущие операции должны добавляться только через module-owned API/native boundary.
