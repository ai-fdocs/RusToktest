...
- Следующий малый Wave 5 availability-semantics slice закрепил case-insensitive matching для inventory backorder policy `continue`: write-result `in_stock`, set/adjust/reserve/check-availability guardrails и admin read-side stock state больше не зависят от регистра policy value.
