...
- [~] journal (`module_operations`) пишется всегда при смене effective state (добавлен integration test `successful_toggle_writes_done_module_operation` для server lifecycle; остаётся parity-покрытие GraphQL/Leptos SSR);
