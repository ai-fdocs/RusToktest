...
- Build pipeline: `BuildService::request_build` now publishes `BuildRequested` via configurable `BuildEventPublisher` (default noop publisher logs skipped dispatch) so async build workers can be wired without changing service API.
