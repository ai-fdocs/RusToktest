...
- Auth/RBAC Casbin shadow prep: при `RUSTOK_RBAC_AUTHZ_MODE=casbin_shadow` permission checks дополнительно запускают parity-shadow path с structured mismatch логом `rbac_engine_mismatch` (`tenant_id`, `user_id`, `resource`, `action`, `relation_decision`, `casbin_decision`), но активным decision-path остаётся relation.
