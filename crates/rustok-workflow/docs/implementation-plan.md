...
- Last checkpoint: FFA-срез workflow admin дополнительно вынес module route toggle/legacy href policy, transport request context, transport error presentation и template create command/name policy в framework-agnostic `admin/src/core/`; `admin/src/transport/mod.rs` принимает core-owned context/command DTO, а `admin/src/ui/leptos.rs` остался тонким adapter-слоем, который связывает Leptos signals/callbacks с `transport/` facade и core-owned commands/view-models.
