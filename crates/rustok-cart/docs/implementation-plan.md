...
- Last checkpoint: Storefront cart UI получил FFA slice: framework-agnostic `core`, thin `transport` facade поверх native-first/GraphQL fallback API и Leptos view без прямого transport/business ownership.
...
- Evidence:
  ...
  - storefront slice выделяет `core` helpers для route/input normalization, UUID validation, adjustment metadata mapping, channel-slug normalization и decrement policy;
  - Leptos `CartView` теперь вызывает thin `transport` facade, а native `#[server]` + GraphQL fallback остаются внутри API adapter layer;
...
- storefront package начал FFA-декомпозицию: pure cart UI policy находится в `storefront/src/core.rs`, Leptos layer использует `storefront/src/transport.rs`, а native-first/GraphQL fallback adapter остаётся в `storefront/src/api.rs`;
...
- [x] начать FFA-разделение storefront package на `core` policy helpers, `transport` facade и Leptos render layer;
