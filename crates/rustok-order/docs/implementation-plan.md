...
- Current phase: ffa_admin_detail_form_state_core_slice
- Last checkpoint: Added core-owned `OrderAdminDetailFormState` and `order_detail_form_state` mapping so payment/shipping/delivery/cancellation editor defaults and fulfillment fallback policy are computed in Leptos-free core; `helpers.rs` now only applies that state to Leptos signals, and the fast order boundary guardrail asserts the split.
