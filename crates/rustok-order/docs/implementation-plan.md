...
- Last checkpoint: Slice 10.1/10.3 продолжен: `order_returns` получили typed resolution-ссылки `resolution_type/refund_id/order_change_id`, чтобы completed return мог указывать на refund/exchange/order-change orchestration без payment side effects внутри `rustok-order`.
