...
- [x] Slice 82: admin table-level empty/count/header state and editor title/submit-button state moved to Leptos-free core (`BlogPostAdminTableViewModel`, `BlogPostAdminFormViewModel`); Leptos table/form adapters now consume render-ready state while keeping DOM, signals, callbacks and i18n lookup local. Evidence: `node scripts/verify/verify-blog-admin-boundary.mjs`; targeted `timeout 25s cargo test -p rustok-blog-admin --lib blog_post_admin_table_view --config profile.dev.debug=0` reached dependency compilation timeout, so no long compile was allowed.
