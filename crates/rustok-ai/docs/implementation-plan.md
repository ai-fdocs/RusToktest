...
- Evidence: `crates/rustok-ai/admin/src/core.rs` now owns Leptos-free request normalization and direct-job payload builders (`parse_csv`, `optional_text`, `alloy_task_payload`, `image_task_payload`, `product_task_payload`, `product_attributes_task_payload`, `blog_task_payload`), `admin/src/transport/mod.rs` owns the current facade over existing native server-function API calls, `admin/src/ui/leptos.rs` is the explicit Leptos adapter consuming `core` + `transport`, and `admin/src/lib.rs` only wires/re-exports module layers.
...
- Next step: continue by splitting native/GraphQL adapters behind `transport/` without removing the existing native server-function + GraphQL/Next.js parallel contract.
