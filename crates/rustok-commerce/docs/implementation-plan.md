...
- Last checkpoint: Reverted the attempted commerce storefront removal of payment/shipping UI helpers because those flows must be moved to owner modules instead of being deleted; the legacy commerce storefront code is preserved as temporary compatibility until `rustok-payment`/`rustok-fulfillment` module-owned storefront UI receives the migrated surfaces.
- Next step: Move the preserved payment-collection and shipping-selection storefront UI/transport slices into owner modules (`rustok-payment` / `rustok-fulfillment`) with local docs + central registry updates in the same change; do not delete the legacy commerce compatibility path before the owner UI exists.
- Open blockers: None.
...
- Evidence:
  ...
  - FFA storefront transport/core split: aggregate checkout route теперь строит `FetchCommerceRequest`, `CartCommandRequest`, `SelectShippingOptionRequest` и commerce-owned context fallback view-model (`tenant/channel/resolution`) в Leptos-free `storefront/src/core/` submodules (`requests`, `presentation`); cart totals/line-items/adjustments stay in `rustok-cart`, payment details stay in `rustok-payment`, order totals stay in `rustok-order`, fulfillment/shipping option details stay in `rustok-fulfillment`, and commerce still carries temporary compatibility rendering/transport for payment-collection and shipping-selection handoff until those UI slices are moved to `rustok-payment`/`rustok-fulfillment` rather than deleted; `storefront/src/transport/mod.rs` owns единственную native-first + GraphQL fallback policy, duplicate combined fallback helpers удалены из `storefront/src/api.rs`, а `native_server_adapter.rs` / `graphql_adapter.rs` являются единственными местами storefront UI package, где вызываются raw native/GraphQL `api::*` functions;
