...
- Last checkpoint: Commerce storefront FFA slice moved payment-collection empty/resolved display fallback into Leptos-free `storefront/src/core/presentation.rs`, keeping `ui/leptos` as a render adapter for payment handoff state.
- Next step: Continue only with small commerce storefront/admin presentation or transport slices that remove real Leptos/host coupling, or harden parity/evidence for existing native + GraphQL checkout/post-order paths without long full-workspace compilation.
- Open blockers: None.
