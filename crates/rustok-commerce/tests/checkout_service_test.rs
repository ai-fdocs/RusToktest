...
// =============================================================================
// Channel-aware inventory visibility through checkout path
// Closes the [ ] items in the "Inventory availability compatibility tail" section
// of crates/rustok-commerce/docs/implementation-plan.md
// =============================================================================

/// Checkout MUST reject a cart line item whose variant uses the `deny` inventory
/// policy when the stock location that holds its inventory is not visible for the
/// cart's channel slug.
#[tokio::test]
...
/// Checkout MUST allow a cart line item whose variant uses the `continue`
/// (backorder) inventory policy even if the stock location is not visible for
/// the cart's channel slug — backorder policy bypasses the availability check.
#[tokio::test]
...
/// Checkout MUST succeed when the stock location is explicitly visible for the
/// cart's channel slug and the variant has sufficient stock with `deny` policy.
#[tokio::test]
