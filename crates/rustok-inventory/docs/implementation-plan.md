...
- Last checkpoint: Добавлен следующий небольшой Wave 5 slice для availability boundary: backend `InventoryService::check_variant_availability` возвращает typed `InventoryAvailabilityCheckResult { available }`, admin model фиксирует endpoint wire shape serde snapshot-ом, native/API facade `inventory/variant/check-availability` проходит tenant/permission checks без GraphQL fallback и вызывается из detail UI через targeted Check availability action. Следующий уточняющий slice закрепил non-negative requested quantity invariant на общем service-level `check_availability` path, чтобы legacy/umbrella callers не обходили validation через старый bool-returning method. Reserve/set/adjust quantity остаются native-only, с typed `InventoryReservationWriteResult { reserved_quantity, available_quantity, in_stock }` и `InventoryQuantityWriteResult { quantity, in_stock }`.
