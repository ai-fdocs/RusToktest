...
- Last checkpoint: Release-reservation доведён до admin detail UI: backend `InventoryService::release_reservation_quantity` возвращает typed `InventoryReservationReleaseWriteResult { released_quantity, available_quantity, in_stock }`, native/API facade `inventory/variant/release-reservation` проходит tenant/permission checks без GraphQL fallback, UI вызывает его через targeted Release reservation action, применяет `available_quantity/in_stock` к detail state, показывает released quantity из typed result, а backend release path не создаёт inventory item/level при failed release и списывает reserved quantity только из существующих levels. Availability check остаётся native-only и вызывается из detail UI через targeted Check availability action; reserve/set/adjust quantity также остаются native-only, с typed `InventoryReservationWriteResult { reserved_quantity, available_quantity, in_stock }` и `InventoryQuantityWriteResult { quantity, in_stock }`.
...
- Evidence:
  ...
  - unit tests покрывают locale fallback, tags extraction, price sale mapping, search normalization, variant title fallback в backend read-side service, service-level non-negative availability request invariant и no-create reservation release error semantics;
